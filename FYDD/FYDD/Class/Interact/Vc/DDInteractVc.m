...
- (void)getMyFootprint{
    @weakify(self)
    [DDHub hub:self.view];
    NSString * url = [NSString stringWithFormat:@"%@/helpEachOther/list?token=%@&size=20&page=%zd&isPublic=0&title=%@",DDAPP_URL,[DDUserManager share].user.token,_currentPage,[yyTrimNullText(_searchText) stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]] ];
