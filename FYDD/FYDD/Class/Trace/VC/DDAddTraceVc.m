...
@interface DDAddTraceVc () <UITextViewDelegate> {
...
- (void)commitDidClick{
    if (yyTrimNullText(_titlePlaceTd.text).length == 0) {
        [DDHub hub:@"请输入足迹标题" view:self.view];
        return;
    }
    if (yyTrimNullText(_titlePlaceTd.text).length > 20) {
        [DDHub hub:@"足迹标题不能超过20个字" view:self.view];
        return;
    }
    if (yyTrimNullText(_contenTextView.text).length == 0) {
        [DDHub hub:@"请输入足迹内容" view:self.view];
        return;
    }
    @weakify(self)
    [DDHub hub:self.view];
    ...
    NSString * url = [NSString stringWithFormat:@"%@/footprint/add?token=%@",DDAPP_2T_URL,[DDUserManager share].user.token];
